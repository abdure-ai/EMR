<div>
    ...
    <div class="space-y-4">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ $followUps->total() }} {{ Str::plural('follow-up', $followUps->total()) }} due
        </p>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <table class="w-full min-w-[760px]">
                    ...
                    <tbody>
                        @forelse ($followUps as $encounter)
                            @php $daysOverdue = $encounter->follow_up_date->diffInDays(today()); @endphp
                            <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                                <td class="px-5 py-4 text-theme-sm text-gray-800 dark:text-white/90 sm:px-6">
                                    <a href="{{ route('patients.show', $encounter->patient) }}" class="hover:text-brand-500">
                                        {{ $encounter->patient->full_name }}
                                    </a>
                                    <span class="font-mono text-xs text-gray-400">{{ $encounter->patient->patient_id }}</span>
                                </td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $encounter->follow_up_date->format('Y-m-d') }}</td>
                                <td class="px-5 py-4 sm:px-6">
                                    @if ($daysOverdue > 0)
                                        <x-ui.badge size="sm" variant="solid" color="error">{{ $daysOverdue }} {{ Str::plural('day', $daysOverdue) }} overdue</x-ui.badge>
                                    @else
                                        <x-ui.badge size="sm" variant="solid" color="warning">Due today</x-ui.badge>
                                    @endif
                                </td>
                                ...
                                <td class="px-5 py-4 text-right text-theme-sm sm:px-6">
                                    <div class="flex items-center justify-end gap-3">
                                        @can('create', \App\Models\Appointment::class)
                                            <a href="{{ route('appointments.create') }}" class="text-brand-500 hover:text-brand-600">Book</a>
                                        @endcan
                                        @if ($canManage)
                                            <button wire:click="dismiss({{ $encounter->id }})" wire:confirm="Dismiss this follow-up? Use this if the patient was reached and no longer needs to come in."
                                                    class="text-gray-500 hover:text-error-500 dark:text-gray-400">
                                                Dismiss
                                            </button>
                                        @endif
                                    </div>
            ...
            <div class="px-5 py-4 sm:px-6">{{ $followUps->links() }}</div>
        </div>
    </div>
</div>
