<div>
    <x-common.page-breadcrumb pageTitle="Medications" />
                ...
                <table class="w-full min-w-[640px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Form</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Strength</p></th>
                    ...
                    <tbody>
                        @forelse ($medications as $medication)
                            <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                                <td class="px-5 py-4 text-theme-sm text-gray-800 dark:text-white/90 sm:px-6">{{ $medication->name }}</td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $medication->form ?: '—' }}</td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $medication->strength ?: '—' }}</td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $medication->price !== null ? number_format($medication->price, 2).' ETB' : '—' }}</td>
                                <td class="px-5 py-4 sm:px-6">
                                    <x-ui.badge variant="solid" :color="$medication->is_active ? 'success' : 'light'">
                                        {{ $medication->is_active ? 'Active' : 'Inactive' }}
                                    </x-ui.badge>
                                </td>
                                <td class="px-5 py-4 text-right text-theme-sm sm:px-6">
                                    @can('update', $medication)
                                        <a href="{{ route('medications.edit', $medication) }}" class="text-brand-500 hover:text-brand-600">Edit</a>
                                        <button wire:click="toggleActive({{ $medication->id }})" class="ml-3 text-gray-500 hover:text-gray-700 dark:text-gray-400">
