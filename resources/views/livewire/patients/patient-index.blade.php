...
<div>
    <x-common.page-breadcrumb :pageTitle="__('Patients')" />

    <div class="space-y-4">
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex flex-wrap items-end gap-3">
                <div class="min-w-[220px] flex-1">
                    <label class="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Search') }}</label>
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('Patient ID, name, or phone...') }}"
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Gender') }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Region') }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Sort by') }}</label>
                ...
                @unless (auth()->user()->hasRole('Practitioner'))
                    <div>
                        <label class="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Show') }}</label>
                        <select wire:model.live="scope" class="{{ $selectClass }}">
                            <option value="today">{{ __('Today only') }}</option>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
            @if (auth()->user()->hasRole('Practitioner'))
                {{ __(':count patient(s) checked in and assigned to you', ['count' => $patients->total()]) }}
            @else
                @if ($scope === 'today')
                    {{ __(':count patient(s) registered today', ['count' => $patients->total()]) }}
                    &middot; <button wire:click="$set('scope', 'all')" type="button" class="text-brand-500 hover:text-brand-600">{{ __('Show all patients') }}</button>
                @else
                    {{ __(':count patient(s) found', ['count' => $patients->total()]) }}
                @endif
            @endif
        </p>
                ...
                <table class="w-full min-w-[820px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Patient ID') }}</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Name') }}</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Gender') }}</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Phone') }}</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Region') }}</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Registered') }}</p></th>
                            <th class="px-5 py-3 text-right sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Actions') }}</p></th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($patients as $patient)
                            ...
                            <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="px-5 py-4 text-theme-sm text-gray-800 dark:text-white/90 sm:px-6">
                                    @if ($identityHidden)
                                        <span class="italic text-gray-400">{{ __('Pending cashier confirmation') }}</span>
                                    @else
                                        {{ $patient->full_name }}
                                    @endif
                                </td>
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <a href="{{ route('patients.show', $patient) }}" title="{{ __('View') }}"
                                        ...
                                        @can('create', \App\Models\QueueEntry::class)
                                            @if (! $patient->has_active_queue_entry && ! $patient->has_pending_visit_invoice)
                                                <a href="{{ route('patients.check-in', $patient) }}" title="{{ __('Check In') }}"
                                                   class="text-gray-400 hover:text-brand-500">
                                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        @endcan
                                        @can('update', $patient)
                                            <a href="{{ route('patients.edit', $patient) }}" title="{{ __('Edit') }}"
                                               class="text-gray-400 hover:text-brand-500">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        @can('delete', $patient)
                                            <button wire:click="delete({{ $patient->id }})"
                                                    wire:confirm="{{ __('Archive :name? This removes them from the active patient list. This isn\'t a permanent delete - the record is preserved.', ['name' => $patient->full_name]) }}"
                                                    title="{{ __('Delete') }}" class="text-gray-400 hover:text-error-500">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-8 text-center text-theme-sm text-gray-500 dark:text-gray-400">
                                    @if (auth()->user()->hasRole('Practitioner'))
                                        {{ __('No patients currently checked in and assigned to you.') }}
                                    @elseif ($scope === 'today' && ! $search && ! $gender && ! $region)
                                        {{ __('No patients registered today yet.') }}
                                        <button wire:click="$set('scope', 'all')" type="button" class="text-brand-500 hover:text-brand-600">{{ __('Show all patients') }}</button>
                                    @else
