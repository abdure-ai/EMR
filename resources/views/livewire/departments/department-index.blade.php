<div>
    ...
    <div class="space-y-4">
            ...
            <div class="flex flex-wrap items-end gap-3">
                <div class="min-w-[220px] flex-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Department name..."
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ $departments->total() }} {{ Str::plural('department', $departments->total()) }} found
        </p>
                ...
                <table class="w-full min-w-[520px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Services</p></th>
                    ...
                    <tbody>
                        @forelse ($departments as $department)
                            <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                                <td class="px-5 py-4 text-theme-sm text-gray-800 dark:text-white/90 sm:px-6">{{ $department->name }}</td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $department->services_count }}</td>
                                <td class="px-5 py-4 sm:px-6">
                                    <x-ui.badge variant="solid" :color="$department->is_active ? 'success' : 'light'">
                                        {{ $department->is_active ? 'Active' : 'Inactive' }}
                                    </x-ui.badge>
                                </td>
                                <td class="px-5 py-4 text-right text-theme-sm sm:px-6">
                                    @can('update', $department)
                                        <button wire:click="toggleActive({{ $department->id }})" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-5 py-8 text-center text-theme-sm text-gray-500 dark:text-gray-400">No departments found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="px-5 py-4 sm:px-6">{{ $departments->links() }}</div>
        </div>
    </div>
</div>
