...
<div>
    <x-common.page-breadcrumb pageTitle="Prescriptions" />
    ...
    <div class="space-y-4">
            ...
            <div class="flex flex-wrap items-end gap-3">
                <div class="min-w-[220px] flex-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Patient ID or name..."
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ $prescriptions->total() }} {{ Str::plural('prescription', $prescriptions->total()) }} found
        </p>
                ...
                <table class="w-full min-w-[720px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><p class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Items</p></th>
                    ...
                    <tbody>
                        @forelse ($prescriptions as $prescription)
                            <tr class="border-b border-gray-100 last:border-0 dark:border-gray-800">
                                <td class="px-5 py-4 text-theme-sm text-gray-800 dark:text-white/90 sm:px-6">
                                    ...
                                    <span class="font-mono text-xs text-gray-400">{{ $prescription->patient->patient_id }}</span>
                                </td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $prescription->practitioner->name ?? '—' }}</td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $prescription->items_count }}</td>
                                <td class="px-5 py-4 sm:px-6">
                                    <x-ui.badge size="sm" variant="solid" :color="$statusColors[$prescription->status] ?? 'light'">
                                        {{ ucfirst($prescription->status) }}
                                    </x-ui.badge>
                                </td>
                                <td class="px-5 py-4 text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">{{ $prescription->created_at->format('Y-m-d H:i') }}</td>
                                <td class="px-5 py-4 text-right sm:px-6">
                                    <a href="{{ route('prescriptions.show', $prescription) }}" title="View" class="inline-flex text-gray-400 hover:text-brand-500">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-8 text-center text-theme-sm text-gray-500 dark:text-gray-400">No prescriptions found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="px-5 py-4 sm:px-6">{{ $prescriptions->links() }}</div>
        </div>
    </div>
</div>
